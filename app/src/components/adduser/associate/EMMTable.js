...
let EMMTable = ({tableOptns}) => {
    ...
    const ManagementRow = (mngt) => {
        ...
        if(mngt.EMMAssigned){
            ...
            mngt.regionList.forEach(rgn => {
                if(rgn.EMMAssigned){mngDisabled = true}
                  rgn.propertyList.forEach(prop => {
                    if(prop.EMAAssigned){mngDisabled = true}
                  })
              })
            mngmtRow =  <tr>
  ...
  const RegionRow = (mng,rgn) => {
        ...
        if(rgn.EMMAssigned){
          ...
          let rgnDisabled = false
          rgn.propertyList.forEach(prop => {
                if(prop.EMAAssigned){rgnDisabled = true}
            })

            regionRow =  <tr key={rgn.bpNumber}>
                            <td><input type="checkbox" onChange={onRegionCBox} checked={rgn.selected} className="" disabled={rgnDisabled?'disabled':''} name="regionBP" value={rgn.bpNumber} /></td>
